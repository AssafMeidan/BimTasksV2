...
namespace BimTasksV2.Helpers.WallSplitter
{
    ...
    public class SplitResult
    {
        ...
        /// <summary>
        /// ElementIds of replacement walls — use these for re-fetching after
        /// TransactionGroup.Assimilate() which can invalidate Wall references.
        /// </summary>
        public List<(ElementId WallId, LayerInfo Layer)> ReplacementIds { get; set; } = new();
    ...
    public static class WallSplitterEngine
    {
        ...
        public static SplitResult SplitWall(Document doc, Wall wall, List<LayerInfo> layers, int? hostedElementTargetLayerIndex)
        {
            ...
            // Validate wall is still accessible (may have been invalidated by a previous split)
            if (!wall.IsValidObject)
            {
                Log.Warning("[WallSplitterEngine] Wall {WallId} is no longer valid, skipping", wall.Id);
                return new SplitResult { Success = false, Message = "Wall is no longer valid in the document." };
            }
            ...
            try
            {
                ...
                using (var tx1 = new Transaction(doc, "Create Replacement Walls"))
                {
                    ...
                    foreach (var layer in layers)
                    {
                        ...
                        // Create the replacement wall (single-layer, no flip needed)
                ...
                var result = new SplitResult
                {
                    ...
                    ReplacementPairs = replacements.ToList(),
                    ReplacementIds = replacements.Select(r => (r.Wall.Id, r.Layer)).ToList(),
