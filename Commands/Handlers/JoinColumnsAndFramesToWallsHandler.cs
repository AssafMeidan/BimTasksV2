...
{
    /// <summary>
    /// Joins geometry between selected structural columns/framing and intersecting walls.
    /// Analogous to JoinAllConcreteWallsAndFloorsGeometryHandler but for columns and frames.
    ...
    /// </summary>
    public class JoinColumnsAndFramesToWallsHandler : ICommandHandler
    {
        private const string CommandName = nameof(JoinColumnsAndFramesToWallsHandler);
            ...
            try
            {
                ...
                if (selectedIds.Count == 0)
                {
                    TaskDialog.Show("No Selection",
                        "Please select structural columns or framing elements.");
                    return;
                }

                // Get structural elements from selection
                var structElements = GetStructuralElements(doc, selectedIds);
                if (structElements.Count == 0)
                {
                    TaskDialog.Show("No Valid Elements",
                        "No structural columns or framing found in selection.");
                    return;
                }

                Log.Information("[{Command}] Found {Count} structural elements", CommandName, structElements.Count);

                // Execute join operation
                using (var trans = new Transaction(doc, "Join Columns & Frames to Walls"))
                {
                    ConfigureFailureHandling(trans);
                    trans.Start();
                    ProcessAllElements(doc, structElements, stats);
                    trans.Commit();
                }
        ...
        private List<Element> GetStructuralElements(Document doc, ICollection<ElementId> selectedIds)
        {
        ...
        private void ProcessAllElements(Document doc, List<Element> elements, JoinStats stats)
        {
            foreach (var elem in elements)
            {
                ...
                TrackCategory(elem, stats);
                ProcessSingleElement(doc, elem, stats);
            }
        }
        ...
        private void ProcessSingleElement(Document doc, Element structElement, JoinStats stats)
        {
            List<Wall> walls;
            try
            {
        ...
        {
            stats.Failed++;
            string categoryName = structElement.Category?.Id.Value == (long)BuiltInCategory.OST_StructuralColumns
                ? "Column" : "Frame";
        ...
        private void ShowResultsDialog(JoinStats stats, long elapsedMs)
        {
            ...
            {
                MainInstruction = $"Processed {stats.ElementsProcessed} structural elements in {elapsedMs}ms",
                MainContent =
                    $"Structural Elements:\n" +
